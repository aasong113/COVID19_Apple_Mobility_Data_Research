...
low_rate <- rbind( indo, cam, kor, tha)
...
reg_viz3+labs(x = "Date", y= "Percent Change in Mobility", title = "Mobility of 4 Low Death Rate Regions")+geom_hline(yintercept = 125, linetype = "dashed", color = "red")+
...
reg_viz_USA+labs(x = "Date", y = "Percent Change in Mobility", title = "Mobility of the USA")+labs(x = "Date", y= "Percent Change in Mobility")+geom_hline(yintercept = 125, linetype = "dashed", color = "red")+annotate(geom = "label", x = ymd("2020/4/10"), y = 125,
           label = "125% Mobility",
           size = 3)
...
reg_viz5 <- high_low%>%
  ggplot(aes(x = date, y = percent_change))+geom_line()+facet_wrap(~region)+geom_smooth()+ylim(0,200)+labs(x = "Date", y= "Percent Change in Mobility", title = "Contrasting Mobility of 4 Regions")+geom_hline(yintercept = 125, linetype = "dashed", color = "red")+ annotate(geom = "label", x = ymd("2020/4/10"), y = 125,label = "125% Mobility",size = 3)
reg_viz5
```
...
ggplot(data = dpp, aes(y = `Deaths per a million people` , x = `Rate of decrease in Mobility`)) + 
  ...
  geom_line(color='red',data = pred_dpp, aes(x=rate, y=death))+ labs(x = "Rate of Mobility Decrease from Peak to Base (Percent/Day)", title = "Effect of Decreasing Mobility Rate on COVID-19 Death Rate")+ annotate(geom = "label", x = 1.2, y = 425,label = "R^2: 0.775, p-value: 0.003",size = 5)
